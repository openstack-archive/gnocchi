...
class DispatcherTest(testtools.TestCase, testscenarios.TestWithScenarios):
    ...
    def test_workflow(self, requests, logger):
        ...
        expected_calls.append(mock.call.post(
            "%(url)s/v1/resource/%(resource_type)s/"
            "%(resource_id)s/entity/%(entity_name)s/measures"
            % url_params,
        ...
        self.assertEqual(len(expected_calls), len(requests.mock_calls))
