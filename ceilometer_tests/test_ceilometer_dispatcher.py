    ...
    def test_workflow(self, requests, logger):
        ...
        if self.resource not in ["create_fail", "patch_fail"]:
            if self.entity != "exists":
                ...
                # TODO(sileht): when we can append one entity to a resource
                # we should add a request for the entity creation here
                # And don't change self.measure
                self.measure = False
            elif self.measure:
                post_responses.append(mock.Mock(status_code=204))
            else:
        ...
        if self.resource in ["create_fail", "patch_fail"]:
            ...
            logger.error.assert_called_with(
                ...
                {'resource_id': self.sample['resource_id'],
                 'status_code': 500,
                 'msg': mock.ANY})
        elif self.entity != "exists":
            # TODO(sileht): this case will disapear once
            # we can append a entity to a resource
            logger.error.assert_called_with(
                mock.ANY,
                {'entity_name': self.sample['counter_name'],
                 'resource_id': self.sample['resource_id'],
                 'status_code': 404})
        elif self.entity == "fail" or not self.measure:
            logger.error.assert_called_with(
                mock.ANY,
                {'entity_name': self.sample['counter_name'],
                 'resource_id': self.sample['resource_id'],
