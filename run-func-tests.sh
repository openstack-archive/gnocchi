...
cleanup(){
    type -t gnocchi_stop >/dev/null && gnocchi_stop
    type -t indexer_stop >/dev/null && indexer_stop
    type -t storage_stop >/dev/null && storage_stop
}
trap cleanup EXIT
...
for storage in ${GNOCCHI_TEST_STORAGE_DRIVERS}; do
    for indexer in ${GNOCCHI_TEST_INDEXER_DRIVERS}; do
        case $storage in
            ...
            swift)
                # FIXME(sileht): just do it! 
                # We return 0 for now to not break the gate
                echo "Unsupported storage backend by functional tests: $GNOCCHI_TEST_STORAGE_DRIVER"
                exit 0
                ;;
            *)
                echo "Unsupported storage backend by functional tests: $GNOCCHI_TEST_STORAGE_DRIVER"
