...
class MetricProcessBase(cotyledon.Service):
    def __init__(self, conf, worker_id, interval_delay=0):
...
class MetricReporting(MetricProcessBase):
    ...
    def __init__(self, conf, worker_id, queues):
        super(MetricReporting, self).__init__(
            conf, worker_id, conf.storage.metric_reporting_delay)
        self.queues = queues
...
class MetricJanitor(MetricProcessBase):
    ...
    def __init__(self, conf, worker_id):
        super(MetricJanitor, self).__init__(
            conf, worker_id, conf.storage.metric_cleanup_delay)
...
class MetricProcessor(MetricProcessBase):
    ...
    def __init__(self, conf, worker_id, queue):
        super(MetricProcessor, self).__init__(
            conf, worker_id, conf.storage.metric_processing_delay)
        self.queue = queue
        self.block_size = 128
...
class MetricdServiceManager(cotyledon.ServiceManager):
    def __init__(self, conf):
        ...
        self.add(self.create_reporter)
        self.add(self.create_janitor)

    def create_processor(self, worker_id):
        queue = self.queues[worker_id - 1]
        return MetricProcessor(self.conf, worker_id, queue)

    def create_reporter(self, worker_id):
        return MetricReporting(self.conf, worker_id, self.queues)

    def create_janitor(self, worker_id):
        return MetricJanitor(self.conf, worker_id)
