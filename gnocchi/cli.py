...
class MetricScheduler(MetricProcessBase):
    ...
    def _get_tasks(self):
        ...
        if not self._tasks or self.last_state != self.partitioner.ring.nodes:
            self.last_state = self.partitioner.ring.nodes
    ...
    def _skip_scheduling(self):
        ...
        if self.prev_sched:
            ...
            if time_left / self.interval_delay > 0.55:
                LOG.warning('Skipping scheduling cycle. Please increase '
                            'workers or scheduling interval')
                return True
            elif time_left / self.interval_delay > 0.1:
                LOG.warning('Delaying scheduling cycle. Please increase '
                            'workers or scheduling interval')
