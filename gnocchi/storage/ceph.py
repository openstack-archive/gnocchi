...
class CephStorage(_carbonara.CarbonaraBasedStorage):
    ...
    OMAP_READ_FLAGS = (rados.LIBRADOS_OPERATION_BALANCE_READS |
    ...
    # can finish before all replicats of set_omap are done, but we don't care,
    # if that occurs next metricd run, will just remove it again, no object
    # with the measure have already been delected by previous, so we are safe
    # and good.
    OMAP_WRITE_FLAGS = rados.LIBRADOS_OPERATION_SKIPRWLOCKS
