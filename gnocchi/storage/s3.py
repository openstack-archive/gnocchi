...
def retry_if_operationaborted(exception):
    return exception.response['Error'].get('Code') == "OperationAborted"
...
class S3Storage(_carbonara.CarbonaraBasedStorage):
        ...
        while response.get('IsTruncated', True):
            ...
            try:
                ...
                if e.response['Error'].get('Code') == "NoSuchBucket":
                    ...
                    return
                raise
            self._bulk_delete(bucket, [c['Key'] for c in response.get('Contents', ())])
        try:
    ...
    def _get_measures(self, metric, timestamp_key, aggregation, granularity,
        ...
        try:
            ...
            raise
        return lz4.loads(response['Body'].read())
    ...
    def _store_unaggregated_timeserie(self, metric, data):
        self.s3.put_object(Bucket=self._bucket_name(metric),
                           Key="none",
                           Body=data)

    def _delete_unaggregated_timeserie(self, metric):
        try:
            self.s3.delete_object(Bucket=self._bucket_name(metric), Key="none")
        except botocore.exceptions.ClientError as e:
            code = e.response['Error'].get('Code')
