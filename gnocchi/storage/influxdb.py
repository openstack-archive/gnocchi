...
class InfluxDBStorage(storage.StorageDriver):
    ...
    def get_measures(self, metric, from_timestamp=None, to_timestamp=None,
                     aggregation='mean'):
        from_timestamp = tz_to_utc(from_timestamp)
        to_timestamp = tz_to_utc(to_timestamp)
        ...
        first_measure_timestamp = self._parse_influxdb_time(result[0], True)
        ...
        time_query = self._make_time_query(
            from_timestamp or first_measure_timestamp, to_timestamp)
        reverse = False
        results = []
        for definition in sorted(
                metric.archive_policy.definition,
                key=operator.attrgetter('granularity'), reverse=reverse):
            subquery = (query +
            ...
            for point in result[metric_id]:
                timestamp = self._parse_influxdb_time(point, True)
    ...
    @staticmethod
    def _parse_influxdb_time(point, fill_tzinfo=False):
        timestamp = timeutils.normalize_time(
            timeutils.parse_isotime(point['time']))
        return (timestamp.replace(tzinfo=iso8601.iso8601.UTC)
                if fill_tzinfo else timestamp)

    def _make_time_query(self, from_timestamp, to_timestamp):
        left_time = "now()"
        if from_timestamp:
            left_time = timeutils.isotime(
                timeutils.normalize_time(from_timestamp), True)

        right_time = None
        if to_timestamp and to_timestamp >= from_timestamp:
            right_time = timeutils.isotime(
                timeutils.normalize_time(to_timestamp), True)
...
def tz_to_utc(ts):
    if isinstance(ts, datetime.datetime) and not ts.tzinfo:
        return ts.replace(tzinfo=iso8601.iso8601.UTC)
    return ts
