...
class SwiftStorage(_carbonara.CarbonaraBasedStorage):
    ...
    def upgrade(self, index):
        ...
        for i in range(sacks):
            self.swift.put_container(self.SACK_PREFIX % i)

    def _store_new_measures(self, metric, data):
        now = datetime.datetime.utcnow().strftime("_%Y%m%d_%H:%M:%S")
        self.swift.put_object(
            self.SACK_PREFIX % metric.sack,
    ...
    def _build_report(self, sacks, details):
        ...
        for i in range(sacks):
            if details:
                headers, files = self.swift.get_container(self.SACK_PREFIX % i,
                                                          full_listing=True)
                for f in files:
    ...
    def list_metric_with_measures_to_process(self, sack):
        headers, files = self.swift.get_container(self.SACK_PREFIX % sack,
                                                  delimiter='/',
                                                  full_listing=True)
    ...
    def _list_measure_files_for_metric_id(self, sack, metric_id):
        headers, files = self.swift.get_container(
            self.SACK_PREFIX % sack, path=six.text_type(metric_id),
            full_listing=True)
        return files

    def delete_unprocessed_measures_for_metric(self, metric):
        files = self._list_measure_files_for_metric_id(metric.sack, metric.id)
        swift.bulk_delete(self.swift, self.SACK_PREFIX % metric.sack, files)
    ...
    def process_measure_for_metric(self, metric):
        ...
        for f in files:
            headers, data = self.swift.get_object(
                self.SACK_PREFIX % metric.sack, f['name'])
