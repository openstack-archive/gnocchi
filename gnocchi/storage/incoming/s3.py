...
class S3Storage(_carbonara.CarbonaraBasedStorage):
    ...
    def _store_new_measures(self, metric, data):
        ...
        self.s3.put_object(
            Bucket=self._sack_name(metric.sack),
    ...
    def delete_unprocessed_measures_for_metric(self, metric):
        files = self._list_measure_files_for_metric_id(metric.sack,
                                                       metric.id)
        s3.bulk_delete(self.s3, self._sack_name(metric.sack), files)

    @contextlib.contextmanager
    def process_measure_for_metric(self, metric):
        sack_name = self._sack_name(metric.sack)
