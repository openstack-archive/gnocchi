...
class InfluxStorage(storage.StorageDriver):
    ...
    def create_metric(self, entity, back_window, archives):
        """Create an entity.
        ...
        data = dict(name=name,
                    columns=columns,
                    points=[[archive['granularity'], archive['points']]
    ...
    def get_measures(self, entity, from_timestamp=None, to_timestamp=None,
        ...
        for archive in self._get_archives(entity):
            ...
            if data:
                ...
                points.extend([
                    Point(timestamp=_as_string(p[ti]),
                          value=p[vi], granularity=archive.granularity)
                    for p in data[0]['points']])
