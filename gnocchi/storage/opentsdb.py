...
def _get_next_stable_point(timestamp, timespan):
    try:
        timestamp = int(timestamp)
        timespan = int(timespan)
    except TypeError:
        ...
        return timestamp - (timestamp % timespan) + timespan
...
class OpenTSDBStorage(storage.StorageDriver):
    ...
    def create_metric(self, metric, back_window, archive_policy):
        ...
        """
        data = []
        for definition in archive_policy:
    ...
    def _prepare_downsampling(self, earliest_ts, latest_ts, metric):
        ...
        definitions = self._get_archive_policy_definitions(metric)
            ...
            for aggr, supported in six.iteritems(self.NATIVE_AGGREGATES):
                if supported:
                    self._write_downsampled_data(
                        metric, supported,
                        definition,
                        _get_previous_stable_point(earliest_ts, timespan),
                        _get_next_stable_point(latest_ts, timespan))
    ...
    def _write_downsampled_data(self, metric, aggregation, definition, from_ts,
        ...
        dps = [{'metric': '%s_%s_%s' % (metric, definition['granularity'],
                                        aggregation),
                'timestamp': int(k), 'value': v,
                'tags': {'resource': 'resource_id'}}
               for k, v in six.iteritems(dps)]
        dps[-1]['timestamp'] = to_ts
    ...
    def get_measures(self, metric, from_timestamp=None, to_timestamp=None,
        ...
        for definition in self._get_archive_policy_definitions(metric):
            ...
            try:
                ...
                dps = [
                    Point(
                        timestamp=datetime.datetime.utcfromtimestamp(float(k)),
