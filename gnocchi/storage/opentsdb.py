...
class OpenTSDBStorage(storage.StorageDriver):
    ...
    def _get_archives(self, metric):
        ...
        for dp in res:
            ...
            archives.append({'granularity': granularity, 'points': points,
                             'back_window': dp['tags']['back_window'],
                             'timespan': dp['tags']['timespan']})
        return archives
    ...
    def add_measures(self, metric, measures):
        ...
        for measure in measures:
            ...
            earliest_ts = min(earliest_ts, measure.timestamp)
            latest_ts = max(latest_ts, measure.timestamp)
        ...
        # get downsampled data and write it to the aggregated metric
        aggr = 'avg'
        # todo(dbelova): DO THAT IN THREADS!
        # python lists appending/extending is atomic operation -> thread-safe
        # opentsdb can process *lots* of API calls at one time
        # todo(dbelova): check somewhere before earliest TS
        # due to the back_window
        # todo(dbelova): process all aggregation functions
        for archive in archives:
            query = self.compose_query(archive['granularity'], metric,
                                       earliest_ts - archive['timespan'],
                                       latest_ts, aggr)
            downsampled_data = self.rest_client.get_query(query)
            dps = downsampled_data['dps']
            dps = [{'metric': '%s_%s' % (metric, archive['granularity']),
                    'timestamp': k, 'value': v,
                    'tags': {'resource': 'resource_id'}}
    ...
    def get_measures(self, metric, from_timestamp=None, to_timestamp=None,
        ...
        for archive in self._get_archives(metric):
            ...
            query = self.compose_query(
                "%s_%s" % (metric, archive['granularity']),
