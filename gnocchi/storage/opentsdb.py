...
class OpenTSDBStorage(storage.StorageDriver):
    ...
    def create_metric(self, metric, back_window, archives):
        ...
        For all metrics archives are stored in 'archives' metric with current
            ...
            for aggr, supported in six.iteritems(self.NATIVE_AGGREGATES):
                if supported:
                    self.executor.submit(
                        _write_downsampled_data, metric, supported, archive,
                        earliest_ts - int(archive['timespan']), latest_ts)
