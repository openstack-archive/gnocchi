...
def _get_previous_stable_point(timestamp, timespan):
    try:
        ...
        return timestamp - (timestamp % timespan)
...
class OpenTSDBStorage(storage.StorageDriver):
    ...
    def create_metric(self, metric):
        ...
        """
        archive_policy = metric.archive_policy
        available_methods = self._serialize_available_aggregation_methods(
            archive_policy.aggregation_methods)
        data = []
        ...
        self.socket_client.put_meter(data, commit=True)

    def _get_archive_policy_definitions(self, metric):
        ...
        aggregation_methods = {}
        if res:
            aggregation_methods = self._deserialize_aggregation_methods(
                res[0]['tags']['aggregation_methods'])
        for dp in res:
            ...
            archive_dps = dp['dps']
            definitions.append({
                'granularity': float(granularity), 'points': points,
    ...
    def add_measures(self, metric, measures):
        ...
        latest_ts = 0
        metric = self._to_hex(metric.name)
        LOG.debug("Post measures %s to metric %s" % (measures, metric))
        ...
        self.executor.submit(self._prepare_downsampling, earliest_ts,
                             latest_ts, metric)

    def _prepare_downsampling(self, earliest_ts, latest_ts, metric):
        time.sleep(self.flush_interval)
        definitions, methods = self._get_archive_policy_definitions(metric)
        for definition in definitions:
            granularity = definition['granularity']
            for aggr, supported in six.iteritems(methods):
                if supported:
                    self._write_downsampled_data(
                        metric, supported,
                        definition,
                        self._get_stable_point(earliest_ts, granularity,
                                               position='previous'),
                        self._get_stable_point(latest_ts, granularity,
                                               position='next'))

    def _write_downsampled_data(self, metric, aggregation, definition, from_ts,
                                to_ts):
        query = self.compose_query(metric, definition['granularity'],
        ...
        metric_name = '%s_%s_%s' % (metric, definition['granularity'],
                                    aggregation)
        dps = [{'metric': metric_name,
                'timestamp': int(k), 'value': v,
                'tags': {'resource': 'resource_id'}}
               for k, v in six.iteritems(dps)]
        self.socket_client.put_meter(dps)
    ...
    def get_measures(self, metric, from_timestamp=None, to_timestamp=None,
        ...
        for definition in definitions:
            # we grab data f downsampled one
            query = self.compose_query(
                "%s_%s_%s" % (metric, definition['granularity'], aggregation),
                from_timestamp=from_timestamp, to_timestamp=to_timestamp,
                aggregation=aggregation)
            LOG.debug("Get measures query: %s" % query)
            ...
            try:
                ...
                dps = [
                    Point(
                        timestamp=pandas.Timestamp.fromtimestamp(float(k)),
                        granularity=definition['granularity'], value=v)
                    for k, v in six.iteritems(dps)]
    ...
    def get_cross_metric_measures(self, metrics, from_timestamp=None,
        ...
        for metric in metrics:
            data = self.get_measures(metric, from_timestamp,
                                     to_timestamp, aggregation)
            if data:
                datas.append(data)

        return carbonara.aggregate_dataframes(datas, len(metrics),
    ...
    def compose_query(metric, granularity=None, from_timestamp=None,
        ...
        # here we need to use granularity from the archive definition
    ...
    @staticmethod
    def _get_stable_point(timestamp, granularity, position='next'):
        shift = 0
        if position == 'next':
            shift = 1
        try:
            ...
            return timestamp - (timestamp % granularity) + shift*granularity

    @staticmethod
    def _serialize_available_aggregation_methods(policy_methods):
        methods = [k for k, v in OpenTSDBStorage.NATIVE_AGGREGATES.items()
                   if k in policy_methods and v]
        return "/".join(methods)

    @staticmethod
    def _deserialize_aggregation_methods(line):
        return dict((method, OpenTSDBStorage.NATIVE_AGGREGATES.get(method))
