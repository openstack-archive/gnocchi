...
class OpenTSDBStorage(storage.StorageDriver):
    ...
    def __init__(self, conf):
        if opentsdb_cl is None:
            raise OpenTSDBClientError('No python-opentsdbclient package found.'
                                      ' Please install it and try again.')
        self.client = opentsdb_cl.Client(conf.storage.opentsdb_host,
                                         conf.storage.opentsdb_port)
            ...
            data.append({'metric': 'archives',
                         ...
                         'tags': {'entity': entity,
                                  'granularity_retention': '%s_%s' % (
                                      archive[0], archive[1])}})
        res_list = self.client.put_meter(data)
    ...
    def _get_archives(self, entity):
        ...
        query = ('start=0&m=sum:archives{entity=%s,granularity_retention=*}'
                 % entity)
        res = self.client.get_query(query)
        res = self.client.process_response(res)
    ...
    def get_measures(self, entity, from_timestamp=None, to_timestamp=None,
        ...
        for archive in self._get_archives(entity):
            ...
            resp = self.client.get_query(query)
            data = self.client.process_response(resp)
            dps = data[0]['dps']
            dps = [
