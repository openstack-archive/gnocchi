...
def decode_xor_floats(encoded, number_of_values):
    ...
    for _ in range(number_of_values - 1):
        ...
        if array[index - 1]:
            ...
            vbits = float_to_int(results[-1])
            vbits ^= num << trailing
            results.append(int_to_float(vbits))
        else:
...
class AggregatedTimeSerie(TimeSerie):
    ...
    def from_dict(cls, d):
        ...
        timestamps = []
        for delta in msgpack.loads(lz4.loads(d.get('timestamps'))):
            prev_timestamp = datetime.timedelta(
                seconds=delta * sampling) + prev_timestamp
            timestamps.append(prev_timestamp)

        values = decode_xor_floats(d.get('values'), len(timestamps))
    ...
    def to_dict(self):
        ...
        return {
            ...
            # LZ4 is more efficient than xor encoding for those values
            'timestamps': lz4.dumps(msgpack.dumps(timestamps)),
            'values': encode_xor_floats(values),
        }
