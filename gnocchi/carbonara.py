...
class AggregatedTimeSerie(TimeSerie):
    ...
    def aggregated(timeseries, aggregation, from_timestamp=None,
        ...
        if fill is not None:
            fill_df = pandas.concat(dataframes, axis=1).fillna(fill)
            single_df = pandas.Series()
            for __, series in fill_df.iteritems():
                single_df = single_df.append(series)
            grouped = pandas.groupby(level=0)
        else:
