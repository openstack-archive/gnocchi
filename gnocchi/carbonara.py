...
class AggregatedTimeSerie(TimeSerie):
    ...
    @classmethod
    def get_next_split_key_datetime(cls, timestamp, sampling):
        """Get the split key of the next split.

        :return: A `datetime.datetime` object.
        """
        return (cls.get_split_key_datetime(timestamp, sampling)
                + (cls.POINTS_PER_SPLIT * sampling))
    ...
    def unserialize(cls, data, start, agg_method, sampling):
        ...
        start = float(start)
        if data:
            s_format = six.indexbytes(data, 0)
            if s_format == ord("c"):
                # Compressed format
    ...
    def serialize(self, start=None, compressed=True):
        ...
        if start is None:
            start = self.get_split_key_datetime(self.first, self.sampling)
        else:
        ...
        if compressed:
            ...
            return 0, b"c" + lz4.dumps(struct.pack(
    ...
    def benchmark(cls):
        ...
        for title, values in [
            ...
            for i in six.moves.range(serialize_times):
                o, s = ts.serialize(compressed=False)
            t1 = time.time()
            ...
            key = pandas.Timestamp(now).value / 10e8
            t0 = time.time()
            for i in six.moves.range(serialize_times):
            ...
            t0 = time.time()
            for i in six.moves.range(serialize_times):
                o, s = ts.serialize(compressed=True)
            t1 = time.time()
