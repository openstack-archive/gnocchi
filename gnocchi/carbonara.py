...
def create_archive_file():
    ...
    parser.add_argument("definition",
                        type=_definition,
                        nargs='+',
                        help="archive definition as sampling,numberOfPoints")
...
def dump_archive_file():
    ...
    ts = TimeSerieArchive.unserialize(args.filename[0].read())
    ...
    map(full_res_table.add_row, ts.full_res_timeserie.ts.iteritems())
    ...
    for idx, agg_ts in enumerate(ts.agg_timeseries):
        ...
        map(table.add_row, agg_ts.ts.iteritems())
