...
class AggregatedTimeSerie(TimeSerie):
        ...
        if data:
            if cls.is_compressed(data):
                ...
                for i, val in itertools.compress(
                        ...
                        deserial[::2]):
                    x.append(val)
                    y.append(start + int(i * sampling))

            y = numpy.array(y, dtype='datetime64[s]')
    ...
    def serialize(self, start, compressed=True):
        ...
        # Get location of one
        locs = (numpy.diff(self.ts.index) // offset_div) * 2,
