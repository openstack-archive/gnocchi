...
class BoundTimeSerie(TimeSerie):
    ...
    def unserialize(cls, data, block_size, back_window):
        ...
        arr = numpy.frombuffer(uncompressed, dtype=[('ts', '<Q'), ('v', '<d')])
        unshuffled = bitshuffle.bitunshuffle(arr)
        timestamps = numpy.cumsum(unshuffled['ts'])
    ...
    def serialize(self):
        ...
        timestamps = numpy.insert(numpy.diff(self.ts.index),
                                  0, self.first.value)
        mixed = numpy.zeros(len(timestamps), dtype=[('ts', '<Q'), ('v', '<d')])
        mixed['ts'] = timestamps
        mixed['v'] = self.ts.values
...
class AggregatedTimeSerie(TimeSerie):
    ...
    def unserialize(cls, data, start, agg_method, sampling):
        ...
        if data:
            if cls.is_compressed(data):
                ...
                shuffled = numpy.frombuffer(uncompressed,
                                            dtype=[('ts', '<H'),
                                                   ('v', '<d')])
                unshuffled = bitshuffle.bitunshuffle(shuffled)
                y = numpy.cumsum(unshuffled['ts'] * sampling) + start
                x = unshuffled['v']
            else:
                # Padded format
                try:
    ...
    def serialize(self, start, compressed=True):
        ...
        if compressed:
            ...
            mixed = numpy.zeros(len(timestamps), dtype=[('ts', '<H'),
                                                        ('v', '<d')])
            mixed['ts'] = timestamps
            mixed['v'] = self.ts.values
