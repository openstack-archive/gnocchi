...
class TimeSerie(object):
    ...
    def __init__(self, ts=None, unique_and_ordered=False):
        if ts is None:
            ts = pandas.Series()
        self.ts = self.clean_ts(ts) if not unique_and_ordered else ts
    ...
    def from_data(cls, timestamps=None, values=None,
                  unique_and_ordered=False):
        return cls(pandas.Series(values, timestamps), unique_and_ordered)

    @classmethod
    def from_tuples(cls, timestamps_values, unique_and_ordered=False):
        return cls.from_data(*zip(*timestamps_values),
                             unique_and_ordered=unique_and_ordered)
...
class BoundTimeSerie(TimeSerie):
    def __init__(self, ts=None, block_size=None, back_window=0,
        ...
        """
        super(BoundTimeSerie, self).__init__(
            ts, unique_and_ordered=unique_and_ordered)
    ...
    @classmethod
    def from_data(cls, timestamps=None, values=None,
                  block_size=None, back_window=0, unique_and_ordered=False):
...
class AggregatedTimeSerie(TimeSerie):
    ...
    def __init__(self, sampling, aggregation_method,
                 ts=None, max_size=None, unique_and_ordered=False):
        ...
        """
        super(AggregatedTimeSerie, self).__init__(
            ts, unique_and_ordered=unique_and_ordered)
    ...
    def from_data(cls, sampling, aggregation_method, timestamps=None,
                  values=None, max_size=None, unique_and_ordered=False):
    ...
    def split(self):
        ...
        ix = numpy.divide(numpy.array(self.ts.index, 'float64'), 10e8)
        keys, counts = numpy.unique((ix // freq) * freq, return_counts=True)
        start = 0
        for key, count in itertools.izip(keys, counts):
            end = start + count
            if key == -0.0:  # float power
                key = abs(key)
