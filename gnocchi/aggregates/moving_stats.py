...
class MovingAverage(aggregates.CustomAggregator):
    ...
    @staticmethod
    def retrieve_data(storage_obj, entity_id, start, stop, window):
        """Retrieves finest-res data available from storage."""
        all_data = storage_obj.get_measures(entity_id, start, stop)
    ...
    def compute(self, storage_obj, entity_id, start, stop, window,
                center=False):
        window = self.check_window_valid(window)
        min_grain, data = self.retrieve_data(storage_obj, entity_id, start,
                                             stop, window)
