...
def aggregate_result(data, func, window, granularity, center=False,
    ...
    def rolling_window(x):
        ...
        if x - left >= start and x + right <= stop:
            if (window / granularity) % 2 == 0 and center:
                offset = datetime.timedelta(seconds=granularity / 2.)
                dslice = [func(ts_sort[x - offset - left: x - offset + right]),
                          func(ts_sort[x + offset - left: x + offset + right])]
                return np.nanmean(dslice)
            elif (window / granularity) % 2 == 1 and center:
                msg = """When doing a centered moving average, the window width
                      must be an even multiple of the granularity."""
