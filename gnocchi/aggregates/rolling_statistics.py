...
class HoltWinters(aggregates.CustomAggregator):
    ...
    def compute(self, storage_object, entity_id, start, stop,
        ...
        if (beta < 0 or beta > 1) or (alpha < 0 or alpha > 1):
            raise aggregates.CustomAggregationFailure(
        ...
        # initialize the first output value as the mean of an initial block of
        # observations (for now take the initial block to be within
        ...
        # set the initial value of the smoothing coefficients.  q is the
        # average time spacing of the data.
        delta = [(data.index[j] - data.index[j - 1]).total_seconds()
                 for j in range(1, len(data))]
        q = np.mean(delta)
        ...
        # procedure called Brown's method that seems to have a more rigorous
        # derivation; not sure if there is any difference practically.
