...
class AggregationFailure(aggregates.AggregationFailure):
    """Error raised when aggregation fails for any reason."""
...
def aggregate_result(data, func, window, center=False, min_periods=1):
    ...
    def rolling_window(x):
        if center:
            ...
            dslice = data[x - datetime.timedelta(seconds=window):x]
