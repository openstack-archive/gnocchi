...
class InvalidResourceAttributeName(voluptuous.Error):
...
class InvalidResourceAttributeValue(voluptuous.Error):
...
class InvalidResourceAttributeOption(voluptuous.Error):
    """Error raised when the resource attribute name is invalid."""
    ...
    def __init__(self, type, name, required, options=None):
        ...
        if options is not None:
            ...
            if fill is None and required:
                raise ValueError("fill options is required for %s if "
                                 "required=True" % name)
