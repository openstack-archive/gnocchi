...
class DispatcherTest(base.BaseTestCase):
    ...
    def test_archive_policy_map_config(self):
        ...
        archive_policy_cfg_file = fileutils.write_to_tempfile(
            content=archive_policy_map,
            prefix="gnocchi_archive_policy_map",
            suffix="yaml")

        d = dispatcher.GnocchiDispatcher(self.conf.conf)
        d.conf.dispatcher_gnocchi.archive_policy_file = archive_policy_cfg_file
        self.assertEqual(
