...
class TestAggregation(tests.TestCase):
    ...
    def test_compute(self):
        ...
        values = aggregate_object.compute(self.index,
                                          self.storage, 'foo',
                                          window='2S')

        start_time = datetime.datetime(2014, 1, 1, 12, 0, 0)
        self._verify_aggregates(values, [55.5, 23], start_time, spacing=1)
    ...
    def test_nans(self):
        self._setup_entity([69, 42, numpy.nan, 44], 'high', spacing=1)
        aggregate_object = rolling_statistics.RollingMean()
        values = aggregate_object.compute(self.index,
                                          self.storage, 'foo',
                                          window='2S')
    ...
    def test_resolution_wrong_keyword(self):
        self._setup_entity([69, 42, 4, 44], 'high', spacing=1)
        self.assertRaises(aggregates.CustomAggregationFailure,
                          rolling_statistics.RollingMean().compute,
                          self.index, self.storage, 'foo', window='2S',
                          resolution='in-between')

        self.storage.delete_entity('foo')

    def test_window_not_mutliple_of_granularity(self):
        self._setup_entity([69, 42, 4, 44], 'medium', spacing=60)
        self.assertRaises(aggregates.CustomAggregationFailure,
                          rolling_statistics.RollingMean().compute,
                          self.index, self.storage, 'foo', window='80S')

        self.storage.delete_entity('foo')

    def test_window_not_specified(self):
        self._setup_entity([69, 42, 4, 44], 'high', spacing=1)
        self.assertRaises(aggregates.CustomAggregationFailure,
                          rolling_statistics.RollingMean().compute,
                          self.index, self.storage, 'foo', window=None)

        self.storage.delete_entity('foo')

    def test_center_even_window(self):
        self._setup_entity([69, 42, 4, 44, 7], 'high', spacing=1)
        aggregate_object = rolling_statistics.RollingMean()
        values = aggregate_object.compute(self.index,
                                          self.storage, 'foo',
                                          window='2S',
        ...
        self._verify_agggregates(values, [48.75, 13.5, 34], start_time,
                                 spacing=1)
        ...
        values = aggregate_object.compute(self.index,
                                          ...
                                          window='4S',
                                          center='True')
        start_time = datetime.datetime(2014, 1, 1, 12, 0, 2)
        self._verify_agggregates(values, [34.875], start_time, spacing=1)

        self.storage.delete_entity('foo')

    def test_center_odd_window(self):
        self._setup_entity([69, 42, 4, 44, 7], 'high', spacing=1)
        aggregate_object = rolling_statistics.RollingMean()
        values = aggregate_object.compute(self.index,
                                          self.storage, 'foo',
                                          ...
                                          center='True')
        start_time = datetime.datetime(2014, 1, 1, 12, 0, 1)
        self._verify_agggregates(values, [42, 4, 44], start_time, spacing=1)
        ...
        values = aggregate_object.compute(self.index,
                                          ...
                                          center='True')
        start_time = datetime.datetime(2014, 1, 1, 12, 0, 2)
        self._verify_agggregates(values, [30], start_time, spacing=1)
        self.storage.delete_entity('foo')
