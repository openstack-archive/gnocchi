...
class TestStatsd(tests_base.TestCase):
    ...
    def _test_gauge_or_ms(self, metric_type, utcnow):
        ...
        self.server.datagram_received("%s:1|%s" % (metric_name, metric_type),
                                      ("127.0.0.1", 12345))
        self.stats.flush()
        ...
        self.server.datagram_received("%s:45|%s" % (metric_name, metric_type),
                                      ("127.0.0.1", 12345))
        self.server.datagram_received("%s:2|%s" % (metric_name, metric_type),
                                      ("127.0.0.1", 12345))
        self.stats.flush()
    ...
    def test_counter(self, utcnow):
        ...
        utcnow.return_value = datetime.datetime(2015, 1, 7, 13, 58, 36)
        self.server.datagram_received("%s:1|c" % metric_name,
                                      ("127.0.0.1", 12345))
        self.stats.flush()
        ...
        utcnow.return_value = datetime.datetime(2015, 1, 7, 13, 59, 37)
        self.server.datagram_received("%s:45|c" % metric_name,
                                      ("127.0.0.1", 12345))
        self.server.datagram_received("%s:2|c|@0.2" % metric_name,
                                      ("127.0.0.1", 12345))
        self.stats.flush()
