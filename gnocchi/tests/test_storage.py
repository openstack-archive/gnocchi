...
class TestStorageDriver(tests.TestCase):
    ...
    def test_add_and_get_aggregated_measures(self):
        self.storage.create_entity("foo", self.archive_policies['low'])
        ...
        values = self.storage.get_aggregated_measures(['foo', 'bar'])
        self.assertEqual(4, len(values))
        ...
        values = self.storage.get_aggregated_measures(
            ['foo', 'bar'],
            from_timestamp='2014-01-01 12:10:00')
        self.assertEqual(1, len(values))
        self.assertEqual(24, values[datetime.datetime(2014, 1, 1, 12, 10, 0)])
    ...
    def test_get_aggregated_measures_unknown_entity(self):
        self.assertRaises(storage.EntityDoesNotExist,
