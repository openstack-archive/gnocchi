...
def ResourceID(value, creator):
    return (six.text_type(value), utils.ResourceUUID(value, creator))
...
def _ResourceSearchSchema():
    user = pecan.request.auth_helper.get_current_user(
        pecan.request.headers)
    ResourceUUID = functools.partial(utils.ResourceUUID, creator=user)
        ...
        voluptuous.All(
            voluptuous.Length(min=0, max=1),
            {
                voluptuous.Any(
                    ...
                    voluptuous.Any({"id": ResourceUUID},
                                   {"id": [ResourceUUID]},
                                   dict)),
                voluptuous.Any(
                    u"and", u"∨",
...
class ResourcesMetricsMeasuresBatchController(rest.RestController):
    ...
    def post(self, create_metrics=False):
        ...
        MeasuresBatchSchema = voluptuous.Schema(
            {functools.partial(utils.ResourceUUID, creator=user):
