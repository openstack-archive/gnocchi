...
def validate(schema, data, required=True, extra=voluptuous.PREVENT_EXTRA):
    try:
...
def deserialize_and_validate(schema, required=True):
    return validate(schema, deserialize(), required=required)
...
class ResourceTypesController(rest.RestController):
    ...
    def post(self):
        ...
        try:
            ...
            abort(409, e)
        set_resp_location_hdr("/v1/resource_type/" + rt.name)
        pecan.response.status = 201
        return rt
...
class ResourcesController(rest.RestController):
    ...
    def post(self):
        ...
        # NOTE(sileht): When the resource is created it have this
        # additional field
