...
def Timespan(value):
    ...
    return six.text_type(datetime.timedelta(seconds=seconds))


class ArchivePolicyItem(object):
    def __init__(self, granularity=None, points=None, timespan=None):
        if ((granularity is None and points is None)
           or (granularity is None and timespan is None)
           or (timespan is None and points is None)):
            raise ValueError(
    ...
    def points(self):
        if self._points is None:
            self._points = (timeparse.timeparse(self._timespan)
                            / self._granularity)
        return self._points
    ...
    def timespan(self):
        if self._timespan is None:
            self._timespan = six.text_type(
                datetime.timedelta(seconds=self._granularity * self._points))
        return self._timespan

    def to_dict(self):
        return {'timespan': self.timespan,
                'granularity': self.granularity,
                'points': self.points}

    @classmethod
    def convert_list_to_dict(cls, archive_policy):
        archive_policy['definition'] = [cls(**d).to_dict()
...
class ArchivePoliciesController(rest.RestController):
    ...
    def post(body):
        ...
        return ArchivePolicyItem.convert_list_to_dict(ap)
    ...
    @pecan.expose('json')
    def get_all(self):
        return list(map(ArchivePolicyItem.convert_list_to_dict,
