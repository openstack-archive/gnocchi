...
def prepare_service(args=None, conf=None,
    ...
    if conf.storage.coordination_url is None:
        ...
        # FIXME(jd) workaround until https://review.openstack.org/272539 is
        # merged
        userpass, _, host = parsed.netloc.partition("@")
        if _ != "@":
            # There's no userpass specified
            host = userpass
        parsed = list(parsed)
        parsed[4] += "&host=%s" % host
