...
resourcehelper = sa.Table(
    'resource',
    sa.MetaData(),
    sa.Column('id', sqlalchemy_utils.types.uuid.UUIDType(binary=True), nullable=False),
    sa.Column('tmp_created_by_user_id', sqlalchemy_utils.types.uuid.UUIDType(binary=True), nullable=True),
    ...
    sa.Column('tmp_project_id', sqlalchemy_utils.types.uuid.UUIDType(binary=True), nullable=True),
    ...
    for tablehelper in [resourcehelper, resourcehistoryhelper]:
        for resource in connection.execute(tablehelper.select()):
            if resource.tmp_created_by_project_id:
                created_by_project_id = str(resource.tmp_created_by_project_id).replace('-','')
            else:
                created_by_project_id = None
            if resource.tmp_created_by_user_id:
                created_by_user_id = str(resource.tmp_created_by_user_id).replace('-','')
            else:
