...
class SQLAlchemyIndexer(indexer.IndexerDriver):
    ...
    resources_history = extension.ExtensionManager(
        'gnocchi.indexer.resources_history')

    _RESOURCE_CLASS_MAPPER = {ext.name: ext.plugin
                              for ext in resources.extensions}

    _RESOURCE_HISTORY_CLASS_MAPPER = {ext.name: ext.plugin
                                      for ext in resources_history.extensions}
    ...
    def _resource_type_to_history_class(self, resource_type):
        if resource_type not in self._RESOURCE_HISTORY_CLASS_MAPPER:
            raise indexer.UnknownResourceType(resource_type)
        return self._RESOURCE_HISTORY_CLASS_MAPPER[resource_type]
    ...
    def _resource_to_dict(resource, with_metrics=False):
        r = dict(resource)
        if with_metrics and (isinstance(resource, Resource) or
    ...
    def update_resource(self, resource_type,
        ...
        resource_cls = self._resource_type_to_class(resource_type)
        resource_history_cls = self._resource_type_to_history_class(
            resource_type)
        session = self.engine_facade.get_session()
        with session.begin():
            ...
            try:
                ...
                # TODO(sileht): retry ourself
                raise ValueError(
                    "The resource have been updated in the meantime, "
                    "please retry...")
