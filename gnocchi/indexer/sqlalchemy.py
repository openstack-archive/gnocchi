...
class ResourceClassMapper(object):
    ...
    def get_classes(self, resource_type):
        if resource_type.state != "created":
...
class SQLAlchemyIndexer(indexer.IndexerDriver):
        ...
        for rt in base.get_legacy_resource_types():
            ...
            if rt.name != "generic":
                try:
                    ...
                    self._set_resource_type_state(rt.name, "error_create")
                    LOG.exception('Fail to create tables for '
                                  'resource_type "%s"', rt.name)
                    continue

            self._set_resource_type_state(rt.name, "created")
    ...
    def create_resource_type(self, resource_type):
        ...
        try:
            ...
            self._set_resource_type_state(resource_type.name, "error_create")
            raise

        self._set_resource_type_state(resource_type.name, "created")
    ...
    def delete_resource_type(self, name):
            ...
            with self.facade.writer() as session:
                ...
                if resource_type.state != "created":
                    raise indexer.UnexpectedResourceTypeState(
                        name, "created", resource_type.state)
        ...
        try:
            ...
            with self.facade.writer() as session:
                resource_type.state = "error_delete"
                session.add(resource_type)
            raise
