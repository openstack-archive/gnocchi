...
class ResourceClassMapper(object):
    ...
    def map_and_create_tables(self, resource_type, facade):
        ...
        try:
            # NOTE(sileht): In theory this is not possible to already have
            # something in the cache because we only use one db connection
            # and table names are uuid. But migration tests are
            # using they own indexer instance on another database and legacy
            # ceilometer resource table are not uuid. So sqla models can
            # already exists in this special case. We can safely reuse them.
            if resource_type.tablename.startswith("rt_"):
                # Not a legacy ceilometer resource
                raise KeyError
