...
class ResourceClassMapper(object):
    ...
    def map_and_create_tables(self, resource_type, connection):
        with self._lock:
            ...
            # FIXME(sileht): Can this fail ? perhaps we need
    ...
    def _safe_table_drop(resource_type, connection, table):
        # TODO(sileht): the reason of the deadlock here is that
        # "DROP TABLE instance" lock the world due to all our foreign keys with
        # resource/resource_history tables.
        # The issue seems only occurs with postgres.
        # I'm guessing that "DROP table instance", lock "instance" and then try
        # to lock "resource", while "select * from instance, resource where..."
        # lock "resource" first and then "instance".
        # So we catch deadlock here and in list_resources() until we have a
        # smarter way to drop theses tables without triggering the deadlock.
...
class SQLAlchemyIndexer(indexer.IndexerDriver):
    ...
    # NOTE(sileht): see command in _safe_table_drop
    @oslo_db.api.retry_on_deadlock
