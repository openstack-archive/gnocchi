...
class QueryTransformer(object):
    ...
    def _handle_binary_op(table, op, nodes):
        ...
        # NOTE(sileht): We hardcode revision_end, because this is
        # not a real column on the table resoure and we can't guess the
        # the column type
        if (value is not None and (
                field_name == 'revision_end' or
                isinstance(attr.type, base.PreciseTimestamp))):
