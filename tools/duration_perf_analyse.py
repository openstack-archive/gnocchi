        ...
        for name in files:
            for method in datas:
                if name.endswith('_%s.csv' % method):
                    ...
                    data.append(pandas.read_csv(filepath))
                    cname = name.replace('_%s.csv' % method, '')
                    data[-1].rename(columns={'Duration': cname}, inplace=True)

    for method in datas:
        merged = pandas.DataFrame(columns=['Index', 'Duration'])
        append = pandas.DataFrame(columns=['Duration'])
        for data in datas[method]:
            data.dropna(axis=1, inplace=True)
            data.drop('Count', axis=1, inplace=True)
            merged = merged.merge(data, on='Index')
            cname = data.columns.values[1]
            data.rename(columns={cname: 'Duration'}, inplace=True)
            append = append.append(data.drop('Index', axis=1))
