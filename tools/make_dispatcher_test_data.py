...
def make_test_data(conn, name, meter_type, unit, volume, random_min,
                   ...
                   end, interval, source='artificial',
                   batch_size=1, resource_count=10,
                   log_file='/tmp/gnocchi_samples.conf'):
    ...
    total_volume = volume
    batch = []
    resource_metadata = prepare_resource_metadata()

    resources = [uuid.uuid4().hex for _ in xrange(resource_count)]
    timedata = []

    while timestamp <= end:
        resource_metadata['flavor']['id'] = random.randint(0, 42)
        ...
        c = sample.Sample(name=name,
                          ...
                          resource_id=resources[
                              random.randint(0, len(resources)-1)],
                          timestamp=timestamp,
        ...
        if len(batch) >= batch_size:
            ts = time.time()
            conn.record_metering_data(batch)
            ts = time.time() - ts
            timedata.append((n, ts, len(batch), float(ts)/len(batch)))
            batch = []
        ...
        if meter_type == 'gauge' or meter_type == 'delta':
            ...
            # volume.
            total_volume = volume
    dump_data(log_file, timedata)
    print('Added %d new events for meter %s.' % (n, name))


def main():
    cfg.CONF(["--config-file", "/opt/stack/gnocchi/etc/gnocchi.conf"],
             project='gnocchi')
    cfg.CONF.dispatcher_gnocchi.url = "http://localhost:8041"
