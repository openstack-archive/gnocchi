...
function _gnocchi_install_grafana {
    ...
    if [ $GRAFANA_PLUGIN_VERSION != "git" ]; then
        (
            cd /var/lib/grafana/plugins
            wget https://github.com/sileht/grafana-gnocchi-datasource/releases/download/${GRAFANA_PLUGIN_VERSION}/grafana-gnocchi-datasource-${GRAFANA_PLUGIN_VERSION}.tar.gz
            tar -xzf grafana-gnocchi-datasource-${GRAFANA_PLUGIN_VERSION}.tar.gz
            rm -f grafana-gnocchi-datasource-${GRAFANA_PLUGIN_VERSION}.tar.gz
        )
        # TODO(sileht): Use grafana cli when plugin have been uploaded to grafana.net
        ...
        (cd /var/lib/grafana/plugins/grafana-gnocchoi-datasource && npm install && ./run-tests.sh) || true
    fi
