...
function start_gnocchi {
    ...
    if [ "$GNOCCHI_USE_KEYSTONE" == "True" ]; then
        ...
        create_archive_policy() { curl -v -X POST -H "X-Auth-Token: $token" -H "Content-Type: application/json" -d "$1" $archive_policy_url ; }
    else
        userid=$(openstack user show admin -f value -c id)
        projectid=$(openstack project show admin -f value -c id)
        create_archive_policy() { curl -v -X POST -H "X-USER-ID: $userid" -H "X-PROJECT-ID: $projectid" -H "Content-Type: application/json" -d "$1" $archive_policy_url ; }
    fi
