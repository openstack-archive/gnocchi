...
if [ -z "$GNOCCHI_DEPLOY" ]; then
    if [ -z "$GNOCCHI_USE_MOD_WSGI" ]; then
        GNOCCHI_DEPLOY=mod_wsgi
    elif [ "$GNOCCHI_USE_MOD_WSGI" == True ]; then
        GNOCCHI_DEPLOY=mod_wsgi
    else
...
function configure_gnocchi {
    ...
    if [ "$GNOCCHI_DEPLOY" == "mod_wsgi" ]; then
        ...
        GNOCCHI_UWSGI_FILE=$GNOCCHI_CONF_DIR/gnocchi-uwsgi-public.ini
        ...
        iniset "$GNOCCHI_UWSGI_FILE" uwsgi threads $(nproc)
