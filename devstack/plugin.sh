...
function configure_gnocchi {
    ...
    if [ "$GNOCCHI_USE_KEYSTONE" == "True" ] ; then
        ...
        if is_service_enabled gnocchi-grafana; then
            ...
            iniset $GNOCCHI_CONF cors allow_headers Content-Type,Cache-Control,Content-Language,Expires,Last-Modified,Pragma,X-Auth-Token,X-Subject-Token,X-User-Id,X-Domain-Id,X-Project-Id
        fi
    else
