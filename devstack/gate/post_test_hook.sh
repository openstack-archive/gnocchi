...
cd $BASE/new/tempest
set +e
sudo -E -H -u stack python tools/install_venv.py
sudo -E -H -u stack ./tools/with_venv.sh pip install -e $GNOCCHI_DIR
sudo -E -H -u stack ./tools/with_venv.sh pip install -e .
sudo -E -H -u stack ./run_tempest.sh -V -- gnocchi
TEMPEST_EXIT_CODE=$?
set -e
