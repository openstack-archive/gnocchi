...
- Listing rados objects for this is not a solution since it takes to much time.
- Using a custom format into a rados object, would enforce us to use a lock
  each time we would change it.

Instead, the xattrs of one empty rados object are used. No lock are needed to
add/remove a xattr.
...
Some objects will be big and so other small depending on how archive policy are
setuped.
