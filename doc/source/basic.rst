...
them. It allows its users to create resources (servers, images, volumes…) with
properties (name, URL, flavors…) and to associate those resources with entities
(CPU usage, bandwidth…) that are going to metered.
...
timestamps and values and computes aggregation according the the defined archive
policies.
