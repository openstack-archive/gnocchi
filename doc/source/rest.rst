...
room or the number of bytes send by a network interface.

An entity only have a few properties: an UUID to identify it, and the archive
...
Gnocchi does not return anything when storing measures, and returns a simple
status code if no errors happened. It is possible to provide any number of
measures.

.. IMPORTANT::

   While it is possible to send any number of (timestamp, value), it is still
   needed to honor aggregation constraints defined by the archive policy used by
   the entity.
...
format can be either an floating number (UNIX epoch) or an ISO8601 formated
timestamp:
...
By default, the aggregated value that are returned use the *mean* aggregation
...
of the level of precision that must be kept when aggregating data. For example,
an item might be defined of 12 points over an hour (one point every 5 minutes),
or a points every 1 hours over 1 day (24 points). An archive policy is defined
by a name and a definition composed of a list of at least one of the previously
described item.
...
and returned, along side the URL of the archive policy. This URL can be used to
...
The *id*, *user_id* are *project_id* attributes must be UUID and are
mandatories. The timestamp describing the lifespan of the resource are not, and
*started_at* is by default set to the current timestamp.
...
All specialized types have their own optional and mandatory attribute, but they all
include attributes from the generic type as well.
