...
`granularity` and `timespan` fields. For example, an item might be defined of 12
points over 1 hour (one point every 5 minutes), or a points every 1 hour over 1
day (24 points).

By default, new measures can only be processed if they have timestamps in the
future or part of the last aggregation period. Such a window size is based on
the largest granularity defined in the archive policy definition. To enlarge
this window size, the `back_window` field can be used to set the number of
period to keep so it is possible to process measures that are older than the
last timestamp sent.
