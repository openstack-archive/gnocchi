...
dump_data(){
    ...
    gnocchi resource list -c id -c type -c project_id -c user_id -c original_resource_id -c started_at -c ended_at -c revision_start -c revision_end
    for resource_id in ${RESOURCE_IDS[@]} $RESOURCE_ID_EXT; do
	    ...
	    gnocchi measures show --aggregation $agg --resource-id $resource_id metric
        done
    done
}
